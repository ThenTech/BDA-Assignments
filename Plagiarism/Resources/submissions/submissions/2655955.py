...
def quick_sort(listy, x, y):
    ...
    if swap_element == 0:
        swap_element = -1
    listy1 = quick_sort(listy[:swap_element], 0, swap_element)
    listy2 = quick_sort(listy[swap_element:], swap_element, len(listy)-1)
    
    return listy1+listy2

def quick_sort_helper(listy):
    return quick_sort(listy, 0, len(listy))