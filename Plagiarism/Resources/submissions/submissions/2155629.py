...
def find_pos(term, corpus):
    ...
    first_corpus_letter_index = 0
    index = 0
    i = 0
    if index == len(corpus) - len(term) + 1:
        return

    for letter in corpus:
        if letter == first_term_letter:
            first_corpus_letter_index = index
            for x in range(index, index + len(term)):
                if corpus[x] == term[i]:
                    i += 1
                    if i == len(term):
                        return first_corpus_letter_index
                else:
                    i = 0
