...
def find_pos(word_to_find, the_string):
    ...
    for letter in the_string:
        if letter == word_to_find[0]:
            ...
            count = 0
            found = True
            for x in range(word_to_find_len):
                if (the_string_len - (position + 1)) < word_to_find_len:
                    return None
                elif word_to_find[x] == the_string[position + x] and found:
                    count += 1
                else:
                    found = False

                if count == word_to_find_len and found:
                    return position
        
        position += 1


print(find_pos("abc", "abababc"))
